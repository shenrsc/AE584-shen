...
for i=1:length(lambdas)
    ...
    %traj_E(:,2) = VT*T_PP;
    xpos_PP = traj_E(:,1)-R.*cos(beta);
    ypos_PP = traj_E(:,2)-R.*sin(beta);
    xpos_PP_all{i}=xpos_PP;
    ypos_PP_all{i}=ypos_PP;
    
    if(i<length(lambdas_1))
        if(length(traj_E(:,1))>length(traj_E1(:,1)))
            traj_E1 = traj_E;
        end
    else
        if(length(traj_E(:,1))>length(traj_E2(:,1)))
            traj_E2 = traj_E;
        end
    end
    ...
    [T_traj_E_CBP,traj_E_CBP] = ode45(ode_traj, T_CBP, y0,opts);
